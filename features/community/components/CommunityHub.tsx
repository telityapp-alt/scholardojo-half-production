...
import { BrainCircuit, ArrowRight, Globe, Layers, SortAsc, Zap, ShieldAlert } from 'lucide-react';
...
export const CommunityHub: React.FC = () => {
    ...
    // FIX: Memory safety for mount timer
    ...
    useEffect(() => {
        ...
        mountTimerRef.current = setTimeout(load, 300);
        ...
        <div className="space-y-6 animate-in fade-in duration-500 pb-24 max-w-7xl mx-auto px-4">
            <div className="bg-sky-50 border-2 border-sky-200 rounded-[28px] p-4 flex flex-col md:flex-row items-center gap-4 shadow-sm shrink-0">
                ...
                <div className="text-center md:text-left">
                    ...
                    <p className="text-sky-700 text-[10px] font-bold opacity-80">Konten dikirim oleh user dan diproses AI. Lakukan verifikasi mandiri sebelum pendaftaran.</p>
                </div>
            </div>
            ...
            <div className="bg-slate-900 rounded-[40px] border-b-[12px] border-black p-8 md:p-10 text-white relative overflow-hidden shadow-2xl">
                 ...
                 <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
                    <div className="max-w-xl">
                        ...
                        <h1 className="text-3xl md:text-5xl font-black tracking-tight leading-none mb-2">Community <span className="text-sky-400">Hub</span></h1>
                        <p className="text-base font-bold text-slate-400 max-w-md">Discover tracks extracted by the collective. Explore a massive vault of community intel.</p>
                    </div>
                    <button onClick={() => setIsScannerOpen(true)} className="bg-[#1cb0f6] text-white px-8 py-5 rounded-[28px] font-black text-lg border-b-[6px] border-[#1899d6] hover:brightness-110 transition-all flex items-center justify-center gap-4 shadow-xl group"><BrainCircuit className="w-8 h-8 group-hover:rotate-12 transition-transform" /> Scan New Brief</button>
                 </div>
            </div>
            ...
            <div className="bg-white p-3 rounded-[28px] border-2 border-slate-200 border-b-[6px] flex flex-col lg:flex-row gap-3 shadow-sm relative z-[50]">
                <div className="lg:w-1/3"><DuoSearch value={searchQuery} onChange={setSearchQuery} placeholder="Search tracks..." /></div>
